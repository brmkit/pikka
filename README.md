...
It started as a fork of [poseidon](https://github.com/MythicAgents/poseidon), which is a cool Go agent for [MythicC2](https://github.com/its-a-feature/mythic) but only for Linux and macOS. I wanted something that could run everywhere, including Windows, but with just the essentials. So, I stripped out most of the stuff from poseidon and added just what was needed to get it running on Windows. Over time, it has become my personal platform to test useful capabilities.
...
Additional capabilities on top of the base poseidon command set:
...
- ~~**execute_assembly**: reflectively loads and executes .NET assemblies in-memory using the CLR. Windows only.~~ (testing)
...
Yes, this repository is developed with AI support.
