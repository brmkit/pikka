...
import (
	...
	"github.com/MythicAgents/pikka/Payload_Type/pikka/agent_code/pkg/utils/enums/InteractiveTask"
)
...
type SendFileToMythicStruct struct {
	...
	// the following are set and used by pikka, Task doesn't use
	// two components used by pikka internally to track and handle chunk responses from Mythic for this specific file transfer
	TrackingUUID         string
...
type Response struct {
	...
	CustomBrowser     *CustomBrowser // custom browser data
...
func (e Response) MarshalJSON() ([]byte, error) {
	...
	// include custom browser data if it exists
	if e.CustomBrowser != nil {
		alias["custom_browser"] = e.CustomBrowser
	}
	return json.Marshal(alias)
}
...
type CustomBrowserEntryChild struct {
	Name            string                 `json:"name"`
	CanHaveChildren bool                   `json:"can_have_children"`
	Metadata        map[string]interface{} `json:"metadata,omitempty"`
}

// CustomBrowserEntry mirrors the C# CustomBrowserEntry
type CustomBrowserEntry struct {
	Name            string                    `json:"name"`
	...
	CanHaveChildren bool                      `json:"can_have_children"`
	Metadata        map[string]interface{}    `json:"metadata,omitempty"`
	Children        []CustomBrowserEntryChild `json:"children,omitempty"`
}

// CustomBrowser mirrors the C# CustomBrowser : IMythicMessage
type CustomBrowser struct {
	BrowserName     string               `json:"browser_name"`
	Host            string               `json:"host"`
	UpdateDeleted   bool                 `json:"update_deleted"`
	SetAsUserOutput bool                 `json:"set_as_user_output"`
	Entries         []CustomBrowserEntry `json:"entries"`
}
