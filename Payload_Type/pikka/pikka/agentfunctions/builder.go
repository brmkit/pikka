...
var payloadDefinition = agentstructs.PayloadType{
	...
	SemVer:        version,
	FileExtension: "",
	Author:        "@brmk, @xorrior, @djhohnstein, @Ne0nd0g, @its_a_feature_",
	SupportedOS: []string{
	...
	Description:                            "A really basic macOS, Linux, and Windows Golang agent.\n",
	SupportedC2Profiles:                    []string{"http", "dynamichttp", "httpx", "dns"},
	...
	BuildParameters: []agentstructs.BuildParameter{
		...
		},
		{
			...
			DefaultValue:  []string{"http", "dynamichttp", "httpx", "dns"},
			GroupName:     "egress",
			UiPosition:    6,
		},
		{
...
func build(payloadBuildMsg agentstructs.PayloadBuildMessage) agentstructs.PayloadBuildResponse {
	...
	pikka_repo_profile := "github.com/MythicAgents/pikka/Payload_Type/pikka/agent_code/pkg/profiles"
	pikka_repo_utils := "github.com/MythicAgents/pikka/Payload_Type/pikka/agent_code/pkg/utils"

	// Build Go link flags that are passed in at compile time through the "-ldflags=" argument
	// https://golang.org/cmd/link/
	ldflags := ""
	if static {
		ldflags += fmt.Sprintf("-extldflags=-static -s -w -X '%s.UUID=%s'", pikka_repo_profile, payloadBuildMsg.PayloadUUID)
	} else {
		ldflags += fmt.Sprintf("-s -w -X '%s.UUID=%s'", pikka_repo_profile, payloadBuildMsg.PayloadUUID)
	}
	ldflags += fmt.Sprintf(" -X '%s.debugString=%v'", pikka_repo_utils, debug)
	ldflags += fmt.Sprintf(" -X '%s.egress_failover=%s'", pikka_repo_profile, egress_failover)
	ldflags += fmt.Sprintf(" -X '%s.failedConnectionCountThresholdString=%v'", pikka_repo_profile, failedConnectionCountThresholdString)
	if egressBytes, err := json.Marshal(egress_order); err != nil {
		...
		ldflags += fmt.Sprintf(" -X '%s.egress_order=%s'", pikka_repo_profile, stringBytes)
	}
	...
	for index, _ := range payloadBuildMsg.C2Profiles {
		...
		for _, key := range payloadBuildMsg.C2Profiles[index].GetArgNames() {
			if key == "AESPSK" {
				...
				//ldflags += fmt.Sprintf(" -X '%s.%s_%s=%s'", pikka_repo_profile, payloadBuildMsg.C2Profiles[index].Name, key, cryptoVal.EncKey)
			} else if key == "headers" {
				...
				//ldflags += fmt.Sprintf(" -X '%s.%s_%s=%v'", pikka_repo_profile, payloadBuildMsg.C2Profiles[index].Name, key, val)
		...
		ldflags += fmt.Sprintf(" -X '%s.%s_%s=%v'", pikka_repo_profile, payloadBuildMsg.C2Profiles[index].Name, "initial_config", initialConfigBase64)
	}
	...
	ldflags += fmt.Sprintf(" -X '%s.proxy_bypass=%v'", pikka_repo_profile, proxyBypass)
	ldflags += " -buildid="
	goarch := "amd64"
	...
	if targetOs == "darwin" {
		...
		if mode != "c-shared" {
			command = strings.Replace(command, "CGO_ENABLED=1", "CGO_ENABLED=0", 1)
		}
		if goarch == "arm64" {
			command += "CC=aarch64-w64-mingw32-gcc CXX=aarch64-w64-mingw32-g++ "
		} else {
	...
	command += "GOGARBLE=* "
	if garble {
		command += "garble -tiny -debug -seed random build "
	} else {
		command += "go build "
	}
	...
	// fix filename for default builds
	nixfilename := "pikka.bin"
	winfilename := "pikka.exe"

	if mode == "default" {
		if targetOs == "windows" {
			payloadBuildResponse.UpdatedFilename = &winfilename
		} else {
			payloadBuildResponse.UpdatedFilename = &nixfilename
		}
	}
	...
	if payloadBytes, err := os.ReadFile(fmt.Sprintf("/build/%s", payloadName)); err != nil {
		...
		fileWriter, err := zipWriter.Create("pikka-darwin-10.12-amd64.a")
		if err != nil {
			payloadBuildResponse.Success = false
		...
		headerWriter, err := zipWriter.Create("pikka-darwin-10.12-amd64.h")
		if err != nil {
			payloadBuildResponse.Success = false
		...
		sharedLib, err := os.Open("./pikka/agent_code/sharedlib/sharedlib-darwin-linux.c")
		if err != nil {
			payloadBuildResponse.Success = false
...
// dummy example function for executing something on a new pikka callback
...
func Initialize() {
	agentstructs.AllPayloadData.Get("pikka").AddPayloadDefinition(payloadDefinition)
	agentstructs.AllPayloadData.Get("pikka").AddBuildFunction(build)
	//agentstructs.AllPayloadData.Get("pikka").AddOnNewCallbackFunction(onNewCallback)
	agentstructs.AllPayloadData.Get("pikka").AddIcon(filepath.Join(".", "pikka", "agentfunctions", "pikka.png"))
}
